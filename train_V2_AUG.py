...
class SpeechNoiseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Randomly remix the existing noise residual at a new SNR.
        # ============================================================
        if self.split == "train":
            noise = noisy - clean

            # Random target SNR: -5 to +10 dB
            aug_snr_db = torch.empty(1).uniform_(-5.0, 10.0).item()

            clean_rms = torch.sqrt(
                torch.mean(clean ** 2) + 1e-10
            )

            noise_rms = torch.sqrt(
                torch.mean(noise ** 2) + 1e-10
            )

            if noise_rms > 1e-8:
                target_noise_rms = (
                    clean_rms /
                    (10.0 ** (aug_snr_db / 20.0))
                )
            ...
            # Random overall gain: -3 to +3 dB
            gain_db = torch.empty(1).uniform_(-3.0, 3.0).item()
            ...
            # Prevent clipping
            peak = max(
                torch.max(torch.abs(clean)).item(),
                torch.max(torch.abs(noisy)).item(),
                1e-8
            )
